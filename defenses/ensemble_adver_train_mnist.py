...
def main(args):
    ...
    if args.self_loader:
        self_inputs = torch.from_numpy(np.load(os.path.join('..','attacks','modelC-pgd-critic-test', 'gen_X.npy'))).float()
        self_labels = torch.from_numpy(np.load(os.path.join('..','attacks','modelC-pgd-critic-test', 'gen_y.npy'))).long()
        # TensorDataset
        self_dataset = TensorDataset(self_inputs, self_labels)
        # Generate the dataloader
        train_loader = DataLoader(self_dataset, batch_size=args.batch_size) 
        print('Use self loader')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--self_loader', default=False, action='store_true',
                        help='Whether to use the self-made loader (Examples generated from AEG)')
