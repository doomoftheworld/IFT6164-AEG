...
from cnn_models import ConfigCNN
from cnn_models import ConfigCNN_BatchNorm
...
"""
Execution command:
python adver_train_mnist.py --seed 42 --mode cln --train_set train --architecture ConfigCNN --nb_conv_block 1 -- save_path ../personal_challenge/mnist/
"""
...
class Config:
    def __init__(self):
        ...
        parser.add_argument('--nb_conv_block', default=1, type = int)
        self.parser = parser
...
def run(args):
    ...
    if "ConfigCNN" in args.architecture:
        archi_name = args.architecture + "_" + str(args.nb_conv_block)
    if args.mode == "cln":
        flag_advtrain = False
        nb_epoch = 10
        model_filename = "mnist_" + archi_name + "_clntrained_%i.pt"%args.seed
    elif args.mode == "adv":
        flag_advtrain = True
        nb_epoch = 90
        model_filename = "mnist_" + archi_name + "_advtrained_%i.pt"%args.seed
    else:
        raise
    ...
    if args.architecture == 'LeNet':
        ...
        print(device)
    elif args.architecture == 'ConfigCNN':
        model = ConfigCNN(args.nb_conv_block, 1,28,28).to(device)
        print(device)
    elif args.architecture == 'ConfigCNN_BatchNorm':
        model = ConfigCNN_BatchNorm(args.nb_conv_block, 1,28,28).to(device)
        print(device)
    else:
