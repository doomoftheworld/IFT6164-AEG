...
class NoBoxAttack(Attack):
    ...
    def eval_fool_rate(self, args, test_loader, adv_models,
            l_test_classif_paths, eval_fn=None):
        fool_rate = 0
        if eval_fn is None:
            with torch.no_grad():
                mean_acc, std_acc, all_acc = eval.eval(args, self,
            ...
            fool_rate = eval_attacker(args, self, "AEG", eval_helpers, args.num_eval_samples)
        print("Final evaluated fool rate :", fool_rate)

    def generate_perturb_samples(self, args, test_loader):
        test_itr = tqdm(enumerate(test_loader),
                         total=len(list(test_loader)))
        generated_adv_x = np.array([]).reshape(0,1,28,28)
        generated_adv_y = []
        for batch_idx, (data, target) in test_itr:
            ...
            if args.not_use_labels:
                # erase the current target to provide
                adv_inputs = self.perturb(x)
            else:
                adv_inputs = self.perturb(x, target=target)
            generated_adv_x = np.vstack([generated_adv_x, adv_inputs.detach().cpu().numpy()])
            generated_adv_y.extend(target.detach().cpu().tolist())
            # Add the normal x
            generated_adv_x = np.vstack([generated_adv_x, x.detach().cpu().numpy()])
            generated_adv_y.extend(target.detach().cpu().tolist())
        generated_adv_y = np.array(generated_adv_y)
        print(generated_adv_x.shape)
        print(generated_adv_y.shape)
        # Save the generated images
        savepath = None
        if args.save_model is None:
            savepath = f'saved_models'
        else:
            savepath = args.save_model
        np.save(os.path.join(savepath,'gen_X.npy'), generated_adv_x)
        np.save(os.path.join(savepath,'gen_y.npy'), generated_adv_y)    
...
def main():
    ...
    parser.add_argument('--command', choices=("eval", "train", "perturb"), default="train")
    ...
    if args.wandb:
        ...
        wandb.init(project='NoBox-Attack-{}-{}'.format(args.dataset, args.namestr),
    ...
    if args.dataset == "mnist":
        ...
        # critic = nn.DataParallel(MadryLeNet(args.nc, args.h, args.w).to(args.dev))
    elif args.dataset == "cifar":
    ...
    if args.command == "eval":
        attacker.load(args)
        attacker.eval_fool_rate(args, test_loader, adv_models,
            l_test_classif_paths, eval_fn=eval_fn)
    ...
    if args.command == "perturb":
        attacker.load(args)
        attacker.generate_perturb_samples(args, test_loader)
